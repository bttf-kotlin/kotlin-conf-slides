...
humain.voyagerDansLeTemps("en Bretagne", 1985) // does not compile
...
marty.location = "BreizhCamp" // does not compile
----
...
[NOTE.speaker]
--
* higher function : prend une fonction en parametre ou renvoi une fonction ou les deux
--
...
[source,kotlin]
----
fun goTo(destination: String): (String) -> Unit = { speaker -> println("${speaker} va à la ${destination}") }
...
[source, kotlin]
----
class Speaker private constructor(val name: String) {
    companion object SpeakerFactory {
        fun create(name: String): Speaker {
            return Speaker(name)
        }
    }
}

val marty = Speaker.SpeakerFactory.create("Marty")
val doc = Speaker.create("Doc")
----
